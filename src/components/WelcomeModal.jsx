...
const WelcomeModal = ({ onClose }) => {
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-center justify-center p-4 transition-all duration-300 ${isAnimating ? 'bg-black/30 backdrop-blur-sm' : 'bg-transparent'
        }`}
    >
      {/* Modal Container */}
      <div
        className={`relative w-full max-w-lg transform transition-all duration-500 ease-out ${isAnimating
        ...
        <div className="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/50 overflow-hidden">
          ...
          <div className="px-6 py-6 -mt-6">
            ...
            <p className="text-center text-[10px] text-gray-400 mt-4">
              BiSınıf - Sınav ve Sınıf Yönetimi
            </p>
          </div>
        </div>
      </div>
